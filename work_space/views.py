...
from .serializers import TaskSerializer, UserSerializer, WorkSpaceSerializer, CreateWorkSpaceSerializer, UserSerializerOne, UserSerializerUpdate, TableSerializer
    ...
    def update(self, request, *args, **kwargs):
        ...
        if owner_id not in member_list:
            raise exceptions.ValidationError(
                {"detail": "no puedes eliminar al dueño del espacio de trabajo"})
        # return super().update(request, *args, **kwargs)
        response = super().update(request, *args, **kwargs)
        workspace = self.get_object()
        members_serializer = UserSerializer(workspace.menbers.all(), many=True)

        # Combinar los datos del espacio de trabajo con los detalles de los miembros
        response.data['menbers'] = members_serializer.data
        return response
...
class tableViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        print(self.kwargs)
        owner_id = int(self.kwargs['owner_id'])
        workspace_id = int(self.kwargs['workspace_id'])
...
class TaskViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        try:
            ...
            raise exceptions.ValidationError(
                {"detail": "El espacio de trabajo no existe"})
        ...
        if not workSpace_menbers.filter(id=owner_id).exists():
            raise exceptions.ValidationError(
                {"detail": "El usuario no pertenece a este espacio de trabajo"})
        if currentWorkSpace.status == False:
            raise exceptions.ValidationError(
                {"detail": "espacio de trabajo no existe o deshabilitado"})
